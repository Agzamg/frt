...
abstract class Animal(var location: Location) {
    val random: Random = SecureRandom()
    val sex: Boolean = random.nextBoolean()
    val fullhp: Int = 10
    var hp: Int = fullhp
...
open class Location(val tree: Tree) {
...
class Trunk(tree: Tree) : Location(tree) {
    ...
    val hollows: Array<Hollow> = Array(random.nextInt(5), { i -> Hollow(tree) })
...
class Roots(tree: Tree) : Location(tree) {
    private val random: Random = SecureRandom()
    val holes: Array<Hole> = Array(random.nextInt(5), { i -> Hole(tree) })
...
class Tree(val treeType: TreeType, val forest: Forest) {
...
class Forest {
    ...
    fun iter() {
        val dead: ArrayList<Animal> = ArrayList<Animal>()
        val born: ArrayList<Animal> = ArrayList<Animal>()
        ...
        for (animal in animals) {
            ...
            for (anim in animal.location.animals) {
                if (anim::class == animal::class && anim.sex != animal.sex && SecureRandom().nextDouble() < 0.12) {
                    anim.breed(born)
                }
            }
        }
    ...
    fun start() {
        ...
        while (population > 0) {
            ...
            println("Day " + iteration + ": " + population.toString() + " animals are alive")
        }
        println("The forest survived for " + iteration + " days")
    }
}